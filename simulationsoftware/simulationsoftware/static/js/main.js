...
// // Event listener to load networks on page load
...
//     const net_id = getNetIdFromUrl();  // Extract the net_id from URL

//     // Fetch user networks for the given net_id
//     fetch(`/get_user_networks/${net_id}`, {
//         method: 'GET',
//         headers: {
//             'Content-Type': 'application/json',
...
//     .then(data => {
//         if (data['status'] === 'success') {
//             selectNetworkElement.innerHTML = "";  // Clear existing options

//             // Add a placeholder option for the select dropdown
...
//             // Add the networks to the dropdown
//             data['network_files'].forEach(networkFile => {
//                 const option = document.createElement("option");
//                 option.value = networkFile.network_name;  // Assuming network_name is returned
//                 option.textContent = networkFile.network_name;  // Display network name
//                 selectNetworkElement.appendChild(option);
//             });
//         } else {
//             console.error("Failed to fetch networks:", data.message);
//         }
//     })
//     .catch(error => console.log("ERROR", error));
// }
...
// //     fetch('/get_all_user_networks/')
// //         .then(res => res.json())
// //         .then(data => {
// //             if (data.status === "success") {
// //                 const dropdown = document.getElementById("networkSelect");
// //                 dropdown.innerHTML = '<option value="">Select a saved network</option>';

// //                 data.network_files.forEach(item => {
// //                     const option = document.createElement("option");
// //                     option.value = `${item.net_id}|${item.network_name}`;// you’ll parse this later
// //                     option.text = `${item.network_name} (Network: ${item.net_id})`;
// //                     dropdown.appendChild(option);
// //                 });
// //             }
// //         })
// //         .catch(error => console.log("Error loading networks:", error));
// // }
...
function saveNetwork(save_network_form_id) {
    ...
    // Dynamically get the logged-in user's ID from a hidden input or global JS variable
    // const userId = document.getElementById("loggedInUserId").value;

    // Get project metadata
    const project_id = document.getElementById("hiddenProjectId").value;
    const category_type = document.getElementById("hiddenCategoryType").value;
    const net_type = document.getElementById("hiddenNetType").value;
    console.log(project_id,category_type,net_type)
...
function put_networks(){
    ...
    const net_type = params.get("net_type");

     fetch(`/get_user_networks?project_id=${project_id}&category_type=${category_type}&network_type=${net_type}`, {
        method:'GET',
        headers:{
            'Content-Type': 'application/json',
            'X-CSRFToken': getCSRFToken()
