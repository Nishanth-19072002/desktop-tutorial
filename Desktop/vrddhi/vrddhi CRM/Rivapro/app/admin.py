...
from .models import UserActivityLog ,GroupButton
...
def get_custom_crud_permissions(*args, **kwargs):
    ...
    try:
        content_type = ContentType.objects.get(app_label='app', model='profile')
...
from django.contrib import admin
from django.contrib.auth.admin import UserAdmin
from .models import UserProfile, Zone,Target
...
class CustomUserAdmin(UserAdmin):
    ...
    list_display = ('username', 'email', 'zone','Target', 'get_groups')
    ...
    def Target(self, obj):
        return obj.userprofile.Target if hasattr(obj, 'userprofile') else None
